...
class TCU:
    ...
    latitude_signal: str = "LocationFrame.Latitude"  # TODO: this must match the name of the latitude frame from you DBC file
    longitude_signal: str = "LocationFrame.Longitude"  # TODO: this must match the name of the latitude frame from you DBC file
    lat: float | None = None
    lng: float | None = None
    ...
    def __init__(self, avp: BehavioralModelArgs) -> None:
        ...
        self.body_can = CanNamespace(
            ...
            restbus_configs=[
                RestbusConfig(
                    restbus_filters=[filters.SenderFilter(ecu_name=TCU.ecu_name)], 
                    delay_multiplier=avp.delay_multiplier,
                    #cycle_time_millis=10 You can override the DBC cycle time here
                )
            ],
        )
