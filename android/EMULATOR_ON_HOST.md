...
4. In terminal D:
    - Linux: Generate the topology (with socketcan), from the root of this repository.
        ...
        -f android/topology/ihu_with_emulator_on_host.instance.yaml \
...
5. Also in terminal D:  
Start the cloud playback, and start the topology from the root of this repository. Unless already signed in start by doing `remotive cloud auth login`. Replace the `project_id` and `session_id` with values from a recording in RemotiveCloud, e.g.`https://console.cloud.remotivelabs.com/p/my-demo/recordings/9459066702917749000`.
