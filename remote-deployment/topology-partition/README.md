...
remotive cloud auth organization set --it   # select your organization
```

> **Security:** the cloud token set on the VM (`/etc/environment`) is readable by anyone with a shell
> on it — use a revocable, least-privilege RemotiveCloud service-account token, not your personal
...
> from any network; password auth is disabled. For non-demo use, restrict it via `ssh_source_cidrs`.
> See [`../SECURITY.md`](../SECURITY.md) for the rationale and the secure setup.
